...
export default function Page() {
  ...
  useEffect(() => {
    ...
    inventory.forEach((item) => { nextDrafts[item.id] = normalizeCategory(item.category); });
    setCategoryDrafts(nextDrafts);
  }, [inventory]);

  useEffect(() => {
    const nextNameDrafts: Record<string, string> = {};
    profiles.forEach((profile) => { nextNameDrafts[profile.id] = profile.name || ''; });
  ...
  async function ensureUserProfile(user: { id: string; email?: string | null }) {
    ...
    const { error: upsertError } = await supabase.from('profiles').upsert({ id: user.id, email, name: baseName }, { onConflict: 'id' });
    if (upsertError) throw upsertError;
    const { data, error } = await supabase.from('profiles').select('id, email, name').eq('id', user.id).maybeSingle();
  ...
  async function fetchInventory() {
    const { data, error } = await supabase.from('inventory_items').select('id, name, current_stock, unit, category, memo').order('name', { ascending: true });
    if (error) throw error;
  ...
  async function fetchLogs() {
    const { data, error } = await supabase.from('inventory_logs').select('id, item_id, action, qty, created_at, note, user_id, user_email, user_name').order('created_at', { ascending: false });
    if (error) throw error;
    setLogs((data ?? []) as InventoryLogRow[]);
  }

  async function fetchProfiles() {
    const { data, error } = await supabase.from('profiles').select('id, email, name').order('email', { ascending: true });
    if (error) throw error;
  ...
  const filteredLogs = useMemo(() => {
    return logs.filter((log) => {
      ...
      const matchesKeyword = logKeyword.trim() === '' || name.toLowerCase().includes(logKeyword.trim().toLowerCase());
  ...
  const productionSourceItems = useMemo(() => {
    if (quickPanel.productionType === '원료생산') return inventory.filter((item) => normalizeCategory(item.category) === '분쇄품');
    if (quickPanel.productionType === '분쇄품생산') return inventory.filter((item) => normalizeCategory(item.category) === '스크랩');
    return [];
  ...
  const quickSummary = useMemo(() => {
    ...
    if (quickPanel.action === '생산') {
      ...
      if (targetItem) parts.push(`결과:${targetItem.name}`);
      else if (quickPanel.targetItemName.trim()) parts.push(`결과:${quickPanel.targetItemName.trim()}`);

      if (quickPanel.productionType === '원료생산' && quickPanel.targetKgQty.trim()) parts.push(`생산량:${quickPanel.targetKgQty.trim()}kg`);
      if (quickPanel.productionType === '분쇄품생산' && quickPanel.targetBagQty.trim()) parts.push(`생산량:${quickPanel.targetBagQty.trim()}bag`);
  ...
  async function insertMessage(content: string, messageType: MessageType) {
    const { error } = await supabase.from('messages').insert({ content, message_type: messageType, user_id: currentUserId, user_email: currentUserEmail, user_name: currentUserName });
    if (error) throw error;
  }

  async function insertLog(itemId: number, action: 'in' | 'out', qty: number, note: string | null = null) {
    const { error } = await supabase.from('inventory_logs').insert({ item_id: itemId, action, qty, note, user_id: currentUserId, user_email: currentUserEmail, user_name: currentUserName });
    if (error) throw error;
  }
  ...
  async function updateInventoryCategory(itemId: number, category: string) {
    const { error } = await supabase.from('inventory_items').update({ category }).eq('id', itemId);
    if (error) throw error;
  }
  ...
  async function createInventoryItem(name: string, category: InventoryCategory, unit: string, initialStock: number) {
  ...
  async function deleteLogAndRollback(log: InventoryLogRow) {
    ...
    if (log.action === 'in') {
      ...
      if (restoredStock < 0) throw new Error('이 로그를 삭제하면 재고가 음수가 돼서 삭제할 수 없어.');
    } else {
      restoredStock = currentStock + Number(log.qty);
    }
  ...
  async function saveUserMessage(content: string, type: MessageType = 'chat') {
    const temp = createTempMessage(content, type, currentUserId, currentUserEmail, currentUserName);
  ...
  async function saveSystemMessage(content: string) {
    const temp = createTempMessage(content, 'system', currentUserId, currentUserEmail, currentUserName);
    addLocalMessage(temp);
    try {
  ...
  function openQuickPanel() {
    setQuickPanel({ ...EMPTY_PANEL, isOpen: true });
    setQuickPanelError('');
    setTimeout(() => { scrollToBottom(); }, 10);
  }
  ...
  function setQuickCategory(category: InventoryCategory) {
    setQuickPanel((prev) => ({ ...prev, category, itemName: '', selectedItemId: null, bagQty: '', kgQty: '' }));
    setQuickPanelError('');
  }
  ...
  function selectQuickItem(item: InventoryItem) {
    setQuickPanel((prev) => ({ ...prev, selectedItemId: item.id, itemName: item.name }));
    setQuickPanelError('');
  }

  function getMatchedItemByName(itemName: string, category?: InventoryCategory | null): InventoryItem | null {
  ...
  async function runQuickPanelAction() {
    ...
    if (!action) { setQuickPanelError('작업을 먼저 선택해줘.'); return; }
    ...
    if (action === '생산') {
      ...
      for (const src of validSources) {
        ...
        if (Number(srcItem.current_stock) < qty) {
          setQuickPanelError(`${srcItem.name} 재고 부족 (현재 ${srcItem.current_stock}${srcItem.unit})`);
          return;
        }
      }
      ...
      if (!targetItem && !typedTargetName) { setQuickPanelError('결과 품목을 선택하거나 새로 입력해줘.'); return; }
      if (!targetItem && typedTargetName) {
        targetItem = await createInventoryItem(typedTargetName, targetCategory, productionType === '원료생산' ? 'kg' : 'bag', 0);
      }
      ...
      if (productionType === '원료생산') {
        ...
        for (const src of validSources) {
          ...
          await insertLog(srcItem.id, 'out', qty, `production_use:원료생산:${targetItem.name}`);
        }
      ...
      if (productionType === '분쇄품생산') {
        ...
        for (const src of validSources) {
          ...
          const qty = Number(src.bagQty);
          await updateInventoryStock(srcItem.id, Number(srcItem.current_stock) - qty);
          await insertLog(srcItem.id, 'out', qty, `production_use:분쇄품생산:${targetItem.name}`);
        }
    ...
    if (action === '재고') {
      ...
      if (!found) { setQuickPanelError('재고 확인할 품목을 입력하거나 선택해줘.'); return; }
      await saveUserMessage(`재고 ${found.name}`, 'chat');
      await saveSystemMessage(`${found.name} 현재 재고는 ${found.current_stock}${found.unit} 이야.`);
      closeQuickPanel();
      return;
    }

    if (action === '입고' || action === '출고') {
      if (!quickPanel.category) { setQuickPanelError('형태를 먼저 선택해줘.'); return; }

      const typedName = quickPanel.itemName.trim();
      let found = selectedItem || getMatchedItemByName(typedName, quickPanel.category);
      if (!typedName && !found) { setQuickPanelError('품목을 입력하거나 선택해줘.'); return; }

      if (quickPanel.category === '원료') {
        const kg = Number(quickPanel.kgQty);
        if (!Number.isFinite(kg) || kg <= 0) { setQuickPanelError('원료는 kg 수량을 입력해줘.'); return; }
        if (!found) {
          if (action === '출고') { setQuickPanelError('없는 품목은 출고할 수 없어.'); return; }
          found = await createInventoryItem(typedName, '원료', 'kg', 0);
        }
        const currentStock = Number(found.current_stock ?? 0);
        if (action === '출고' && currentStock < kg) { setQuickPanelError(`${found.name} 재고 부족 (현재 ${currentStock}${found.unit})`); return; }
        const newStock = action === '입고' ? currentStock + kg : currentStock - kg;
        await saveUserMessage(`${action} ${quickPanel.category} ${found.name} ${kg}kg`, 'chat');
        await updateInventoryStock(found.id, newStock);
        await insertLog(found.id, action === '입고' ? 'in' : 'out', kg);
        await saveSystemMessage(willCreateNewItem ? `${found.name} 새 품목 추가 후 ${kg}kg ${action} 완료.` : `${found.name} ${kg}kg ${action} 완료.`);
        closeQuickPanel();
        await fetchAll();
        return;
      }
      ...
      if (!Number.isFinite(bagQty) || bagQty <= 0) { setQuickPanelError('분쇄품/스크랩은 bag 수를 입력해줘.'); return; }
      if (kgQty !== null && (!Number.isFinite(kgQty) || kgQty < 0)) { setQuickPanelError('kg는 비워두거나 숫자로 입력해줘.'); return; }
      if (!found) {
        if (action === '출고') { setQuickPanelError('없는 품목은 출고할 수 없어.'); return; }
        found = await createInventoryItem(typedName, quickPanel.category, 'bag', 0);
      }
      const currentStock = Number(found.current_stock ?? 0);
      if (action === '출고' && currentStock < bagQty) { setQuickPanelError(`${found.name} 재고 부족 (현재 ${currentStock}${found.unit})`); return; }
      const newStock = action === '입고' ? currentStock + bagQty : currentStock - bagQty;
      const kgText = kgQty !== null && kgQty > 0 ? ` / ${kgQty}kg` : '';
      await saveUserMessage(`${action} ${quickPanel.category} ${found.name} ${bagQty}bag${kgText}`, 'chat');
      await updateInventoryStock(found.id, newStock);
      await insertLog(found.id, action === '입고' ? 'in' : 'out', bagQty);
      await saveSystemMessage(willCreateNewItem ? `${found.name} 새 품목 추가 후 ${bagQty}bag${kgText} ${action} 완료.` : `${found.name} ${bagQty}bag${kgText} ${action} 완료.`);
      closeQuickPanel();
      await fetchAll();
    }
  }
  ...
  async function handleSaveCategory(itemId: number) {
    ...
    if (!category || !CATEGORY_OPTIONS.includes(category as InventoryCategory)) { setErrorText('카테고리를 선택해줘.'); return; }
    try {
  ...
  async function handleClearMessages() {
    ...
    try {
      setErrorText('');
      // TODO: 나중에 아래 delete 코드 지우고 setMessages([]) 만 남기기
  ...
  async function handleUpdateItem(itemId: number) {
    ...
    try {
      setErrorText('');
      const { error } = await supabase.from('inventory_items').update({ name, current_stock: stock, category, memo: editingItemMemo }).eq('id', itemId);
      if (error) throw error;
  ...
  async function processLegacyCommand(raw: string) {
    ...
    if (command === '/도움말') { await saveSystemMessage('사용 가능 명령어: / 또는 /도움말 또는 기존 /재고 /입고 /출고'); return; }
    if (!itemName) { await saveSystemMessage('품목명을 같이 입력해줘. 예: /재고 PP백색'); return; }

    const normalizedItemName = itemName.trim().toLowerCase();
    const found = inventory.find((item) => item.name.trim().toLowerCase() === normalizedItemName);
    if (!found) { await saveSystemMessage(`"${itemName}" 품목을 찾지 못했어.`); return; }

    if (command === '/재고') { await saveSystemMessage(`${found.name} 현재 재고는 ${found.current_stock}${found.unit} 이야.`); return; }
    if ((command === '/입고' || command === '/출고') && (!Number.isFinite(qty) || qty <= 0)) { await saveSystemMessage('수량은 숫자로 입력해줘. 예: /입고 PP백색 100'); return; }
    ...
    if (command === '/출고') {
      if (Number(found.current_stock) < qty) { await saveSystemMessage(`${found.name} 재고 부족 (현재 ${found.current_stock}${found.unit})`); return; }
      const newStock = Number(found.current_stock) - qty;
      await updateInventoryStock(found.id, newStock);
  ...
  async function handleSend() {
    ...
    try {
      ...
      setErrorText('');
      setQuickPanelError('');
      if (trimmed === '/') { setInput(''); openQuickPanel(); return; }
      setInput('');
      if (trimmed.startsWith('/')) { await processLegacyCommand(trimmed); return; }
  ...
  return (
    <main className="min-h-screen bg-neutral-100 text-neutral-900">
      <div className="mx-auto flex min-h-screen w-full max-w-md flex-col bg-neutral-50 shadow-sm">
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <h1 className="text-lg font-bold tracking-tight">
                {activeTab === 'chat' ? '채팅' : activeTab === 'stock' ? '재고' : '입출고 로그'}
              </h1>
            </div>
            <button onClick={() => void handleLogout()} disabled={loggingOut} className="rounded-2xl border border-neutral-200 bg-white px-3 py-2 text-xs font-semibold text-neutral-700 disabled:opacity-50">
        ...
        <section className={cn('flex-1', activeTab === 'chat' ? 'pb-[200px]' : 'overflow-y-auto pb-24')}>
          ...
          {!loading && activeTab === 'chat' && (
            <div ref={chatScrollRef} className="overflow-y-auto px-3 py-4" style={{ height: '100%' }}>
              <div className="mb-3 rounded-2xl border border-neutral-200 bg-white px-4 py-3">
                ...
                <p className="mt-1 text-xs leading-5 text-neutral-500">채팅창 왼쪽 ⚡ 버튼을 누르면 빠른입력 열림</p>
              </div>
                  ...
                  return (
                    <div key={message.id} className={cn('flex', isUser ? 'justify-end' : 'justify-start')}>
                      <div className="max-w-[84%]">
                        <div className={cn('rounded-2xl px-4 py-3 text-sm leading-6 shadow-sm', isUser && !isCommand && 'rounded-br-md bg-neutral-900 text-white', isCommand && 'rounded-br-md border border-blue-200 bg-blue-50 text-blue-900', !isUser && 'rounded-bl-md border border-neutral-200 bg-white text-neutral-800')}>
          ...
          {!loading && activeTab === 'stock' && (
            <div className="px-3 py-4">
              <div className="mb-3 flex items-center justify-between gap-2">
                <button onClick={() => setStockManageMode(false)} className={cn('flex-1 rounded-2xl border px-3 py-3 text-sm font-medium', !stockManageMode ? 'border-neutral-900 bg-neutral-900 text-white' : 'border-neutral-200 bg-white text-neutral-600')}>재고 보기</button>
                <button onClick={() => setStockManageMode(true)} className={cn('flex-1 rounded-2xl border px-3 py-3 text-sm font-medium', stockManageMode ? 'border-neutral-900 bg-neutral-900 text-white' : 'border-neutral-200 bg-white text-neutral-600')}>관리</button>
              </div>
                  ...
                  <div className="rounded-3xl border border-neutral-200 bg-white p-4 shadow-sm">
                    {currentUserEmail === ADMIN_EMAIL && (
                      <div className="mb-4 rounded-3xl border border-red-200 bg-red-50 p-4 shadow-sm">
                        ...
                        <button onClick={() => void handleClearMessages()} className="w-full rounded-2xl bg-red-600 px-4 py-3 text-sm font-semibold text-white">채팅 기록 초기화</button>
                      </div>
                    )}
                        ...
                        profiles.map((profile) => (
                          <div key={profile.id} className="rounded-2xl border border-neutral-200 bg-neutral-50 p-3">
                            ...
                            <div className="mt-2 flex gap-2">
                              <input value={userNameDrafts[profile.id] ?? ''} onChange={(e) => setUserNameDrafts((prev) => ({ ...prev, [profile.id]: e.target.value }))} placeholder="직원 이름" className="flex-1 rounded-2xl border border-neutral-200 bg-white px-4 py-3 text-sm outline-none placeholder:text-neutral-400 focus:border-neutral-400" />
                              <button onClick={() => void handleSaveProfileName(profile.id)} disabled={savingProfileId === profile.id} className="rounded-2xl border border-neutral-200 bg-white px-4 py-3 text-sm font-semibold text-neutral-700 disabled:opacity-50">
                      ...
                      <div key={item.id} className="rounded-3xl border border-neutral-200 bg-white p-4 shadow-sm">
                        {editingItemId === item.id ? (
                          <div className="space-y-2">
                            ...
                            <div className="grid grid-cols-3 gap-2">
                              {CATEGORY_OPTIONS.map((category) => (
                                <button key={category} onClick={() => setCategoryDrafts((prev) => ({ ...prev, [item.id]: category }))} className={cn('rounded-2xl border px-3 py-3 text-sm font-medium', categoryDrafts[item.id] === category ? 'border-neutral-900 bg-neutral-900 text-white' : 'border-neutral-200 bg-neutral-50 text-neutral-700')}>
                                  {category}
                                </button>
                              ))}
                            </div>
                          ...
                          <div>
                            <div className="mb-3 flex items-center justify-between">
                              ...
                              <button onClick={() => { setEditingItemId(item.id); setEditingItemName(item.name); setEditingItemStock(String(item.current_stock)); setEditingItemMemo(item.memo ?? ''); }} className="rounded-2xl border border-neutral-200 bg-neutral-50 px-3 py-2 text-sm font-semibold text-neutral-700">수정</button>
                            </div>
                          </div>
                        )}
                      </div>
          ...
          {!loading && activeTab === 'logs' && (
            <div className="px-3 py-4">
              <div className="mb-3 space-y-2">
                <input value={logKeyword} onChange={(e) => setLogKeyword(e.target.value)} placeholder="품목 검색" className="w-full rounded-2xl border border-neutral-200 bg-white px-4 py-3 text-sm outline-none placeholder:text-neutral-400 focus:border-neutral-400" />
                <div className="flex gap-2">
                  <button onClick={() => setLogFilter('all')} className={cn('flex-1 rounded-2xl border px-3 py-3 text-sm font-medium', logFilter === 'all' ? 'border-neutral-900 bg-neutral-900 text-white' : 'border-neutral-200 bg-white text-neutral-600')}>전체</button>
                  <button onClick={() => setLogFilter('in')} className={cn('flex-1 rounded-2xl border px-3 py-3 text-sm font-medium', logFilter === 'in' ? 'border-emerald-600 bg-emerald-600 text-white' : 'border-neutral-200 bg-white text-neutral-600')}>입고</button>
                  <button onClick={() => setLogFilter('out')} className={cn('flex-1 rounded-2xl border px-3 py-3 text-sm font-medium', logFilter === 'out' ? 'border-red-600 bg-red-600 text-white' : 'border-neutral-200 bg-white text-neutral-600')}>출고</button>
                </div>
              </div>

              <div className="space-y-3">
                {filteredLogs.length === 0 ? (
                  <div className="rounded-2xl border border-dashed border-neutral-300 bg-white px-4 py-8 text-center text-sm text-neutral-500">표시할 로그가 없어.</div>
                ) : (
                      ...
                      <div key={log.id} className="rounded-3xl border border-neutral-200 bg-white p-4 shadow-sm">
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            ...
                            {(log.user_name || log.user_email) && <p className="mt-1 truncate text-xs text-neutral-500">작성: {log.user_name || log.user_email}</p>}
                        ...
                        <button onClick={() => void handleDeleteLog(log)} disabled={deletingLogId === log.id} className="mt-4 w-full rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-700 disabled:opacity-50">
            ...
            {quickPanel.isOpen && (
              <div className="mb-2 rounded-3xl border border-neutral-200 bg-white p-3 shadow-sm">
                <div className="mb-3 flex items-center justify-between">
                  ...
                  <button onClick={() => closeQuickPanel()} className="rounded-full border border-neutral-200 px-2.5 py-1 text-xs text-neutral-600">닫기</button>
                </div>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      {(['재고', '입고', '출고', '생산'] as QuickAction[]).map((action) => (
                        <button key={action} onClick={() => setQuickAction(action)} className={cn('rounded-2xl border px-2 py-3 text-xs font-medium', quickPanel.action === action ? 'border-neutral-900 bg-neutral-900 text-white' : 'border-neutral-200 bg-neutral-50 text-neutral-700')}>
                          {action}
                        </button>
                      ))}
                    </div>
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {(['원료생산', '분쇄품생산'] as ProductionType[]).map((type) => (
                            <button key={type} onClick={() => setProductionType(type)} className={cn('rounded-2xl border px-3 py-3 text-sm font-medium', quickPanel.productionType === type ? 'border-neutral-900 bg-neutral-900 text-white' : 'border-neutral-200 bg-neutral-50 text-neutral-700')}>
                              {type}
                            </button>
                          ))}
                        </div>
                        ...
                        <>
                          <div>
                            <p className="mb-2 text-xs text-neutral-500">
                              사용 품목 {quickPanel.productionType === '원료생산' ? '(분쇄품)' : '(스크랩)'}
                            </p>
                          ...
                          <div>
                            <p className="mb-2 text-xs text-neutral-500">결과 품목 {quickPanel.productionType === '원료생산' ? '(원료)' : '(분쇄품)'}</p>
                            <input value={quickPanel.targetItemName} onChange={(e) => { setQuickPanel((prev) => ({ ...prev, targetItemName: e.target.value, targetItemId: null })); setQuickPanelError(''); }} placeholder="결과 품목 직접 입력 가능" className="mb-2 w-full rounded-2xl border border-neutral-200 bg-white px-4 py-3 text-sm outline-none placeholder:text-neutral-400 focus:border-neutral-400" />
                            <div className="flex flex-wrap gap-2">
                              {productionTargetItems.map((item) => (
                                <button key={item.id} onClick={() => setQuickPanel((prev) => ({ ...prev, targetItemId: item.id, targetItemName: item.name }))} className={cn('rounded-full border px-3 py-2 text-sm', quickPanel.targetItemId === item.id ? 'border-neutral-900 bg-neutral-900 text-white' : 'border-neutral-200 bg-neutral-50 text-neutral-700')}>
                                  {item.name}
                                </button>
                              ))}
                            </div>
                          ...
                          {quickPanel.productionType === '원료생산' && (
                            <div>
                              ...
                              <input value={quickPanel.targetKgQty} onChange={(e) => { setQuickPanel((prev) => ({ ...prev, targetKgQty: e.target.value })); setQuickPanelError(''); }} placeholder="생산된 원료 kg" inputMode="decimal" className="w-full rounded-2xl border border-neutral-200 bg-white px-4 py-3 text-sm outline-none placeholder:text-neutral-400 focus:border-neutral-400" />
                            </div>
                          )}
                          ...
                          {quickPanel.productionType === '분쇄품생산' && (
                            <div>
                              ...
                              <input value={quickPanel.targetBagQty} onChange={(e) => { setQuickPanel((prev) => ({ ...prev, targetBagQty: e.target.value })); setQuickPanelError(''); }} placeholder="생산된 분쇄품 bag" inputMode="numeric" className="w-full rounded-2xl border border-neutral-200 bg-white px-4 py-3 text-sm outline-none placeholder:text-neutral-400 focus:border-neutral-400" />
                            </div>
                          )}
                        </>
                      )}
                  ...
                  {(quickPanel.action === '입고' || quickPanel.action === '출고') && (
                    <div>
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {CATEGORY_OPTIONS.map((category) => (
                          <button key={category} onClick={() => setQuickCategory(category)} className={cn('rounded-2xl border px-3 py-3 text-sm font-medium', quickPanel.category === category ? 'border-neutral-900 bg-neutral-900 text-white' : 'border-neutral-200 bg-neutral-50 text-neutral-700')}>
                            {category}
                          </button>
                        ))}
                      </div>
                  ...
                  {quickPanel.action && quickPanel.action !== '생산' && (
                    <div>
                      ...
                      <input value={quickPanel.itemName} onChange={(e) => { setQuickPanel((prev) => ({ ...prev, itemName: e.target.value, selectedItemId: null })); setQuickPanelError(''); }} placeholder="품목명 입력" className="mb-2 w-full rounded-2xl border border-neutral-200 bg-white px-4 py-3 text-sm outline-none placeholder:text-neutral-400 focus:border-neutral-400" />

                      <div className="flex flex-wrap gap-2">
                        {quickPanelItems.length === 0 ? (
                          <p className="text-xs text-neutral-500">선택 가능한 기존 품목이 없어. 직접 입력해도 돼.</p>
                        ) : (
                          <div className="relative w-full">
                            <button onClick={() => setItemModalOpen((prev) => !prev)} className="w-full rounded-2xl border border-neutral-200 bg-white px-4 py-3 text-left text-sm text-neutral-700">
                  ...
                  {(quickPanel.action === '입고' || quickPanel.action === '출고') && (
                    quickPanel.category === '원료' ? (
                      <div>
                        ...
                        <input value={quickPanel.kgQty} onChange={(e) => { setQuickPanel((prev) => ({ ...prev, kgQty: e.target.value })); setQuickPanelError(''); }} placeholder="kg 수량" inputMode="decimal" className="w-full rounded-2xl border border-neutral-200 bg-white px-4 py-3 text-sm outline-none placeholder:text-neutral-400 focus:border-neutral-400" />
                      </div>
                    ) : quickPanel.category ? (
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          <p className="mb-2 text-xs text-neutral-500">bag</p>
                          <input value={quickPanel.bagQty} onChange={(e) => { setQuickPanel((prev) => ({ ...prev, bagQty: e.target.value })); setQuickPanelError(''); }} placeholder="bag 수" inputMode="numeric" className="w-full rounded-2xl border border-neutral-200 bg-white px-4 py-3 text-sm outline-none placeholder:text-neutral-400 focus:border-neutral-400" />
                        </div>
                        <div>
                          <p className="mb-2 text-xs text-neutral-500">kg (선택)</p>
                          <input value={quickPanel.kgQty} onChange={(e) => { setQuickPanel((prev) => ({ ...prev, kgQty: e.target.value })); setQuickPanelError(''); }} placeholder="없으면 비워둬" inputMode="decimal" className="w-full rounded-2xl border border-neutral-200 bg-white px-4 py-3 text-sm outline-none placeholder:text-neutral-400 focus:border-neutral-400" />
                        </div>
                      </div>
                    ) : null
                  )}
                  ...
                  {quickPanelError && (
                    <div className="rounded-2xl border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">{quickPanelError}</div>
                  )}

                  <div className="grid grid-cols-2 gap-2">
                    <button onClick={() => closeQuickPanel()} className="rounded-2xl border border-neutral-200 bg-white px-4 py-3 text-sm font-medium text-neutral-700">취소</button>
                    <button onClick={() => void handleQuickPanelExecute()} disabled={sending} className="rounded-2xl bg-neutral-900 px-4 py-3 text-sm font-semibold text-white disabled:opacity-50">
                      {sending ? '처리중' : '실행'}
                    </button>
                  </div>
                </div>
            ...
            <div className="rounded-3xl border border-neutral-200 bg-white p-2 shadow-lg">
              <div className="flex items-end gap-2">
                <label className="flex h-11 w-11 shrink-0 cursor-pointer items-center justify-center rounded-2xl border border-neutral-200 bg-neutral-50 text-lg">
                  ...
                  <input type="file" accept="image/*" className="hidden" onChange={(e) => { const file = e.target.files?.[0]; if (file) void handleImageUpload(file); e.target.value = ''; }} />
                </label>

                <button onClick={() => openQuickPanel()} type="button" className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-neutral-200 bg-neutral-50 text-lg" aria-label="빠른입력 열기" title="빠른입력">
                  ⚡
                </button>
                ...
                <textarea
                  ...
                  onChange={(e) => {
                    ...
                    setInput(value);
                    if (value.trim() === '/' && !quickPanel.isOpen) openQuickPanel();
                  }}
        ...
        <nav className="fixed bottom-0 left-1/2 z-30 w-full max-w-md -translate-x-1/2 border-t border-neutral-200 bg-white/95 pb-[calc(env(safe-area-inset-bottom)+8px)] pt-2 backdrop-blur">
          <div className="grid grid-cols-3 gap-2 px-3">
            <TabButton label="채팅" icon="💬" active={activeTab === 'chat'} onClick={() => setActiveTab('chat')} />
            <TabButton label="재고" icon="📦" active={activeTab === 'stock'} onClick={() => setActiveTab('stock')} />
            <TabButton label="입출고" icon="🧾" active={activeTab === 'logs'} onClick={() => setActiveTab('logs')} />
          </div>
        </nav>
      </div>
          ...
          <div className="w-full max-w-md rounded-3xl bg-white p-4 pb-8" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="max-h-80 overflow-y-auto space-y-2">
              {quickPanelItems.map((item) => (
                <button key={item.id} onClick={() => { selectQuickItem(item); setItemModalOpen(false); }} className={cn('w-full rounded-2xl border px-4 py-3 text-left text-sm font-medium', quickPanel.selectedItemId === item.id ? 'border-neutral-900 bg-neutral-900 text-white' : 'border-neutral-200 bg-neutral-50 text-neutral-700')}>
                  {item.name}
                </button>
              ))}
            </div>
...
function TabButton({ label, icon, active, onClick }: { label: string; icon: string; active: boolean; onClick: () => void; }) {
  return (
    <button onClick={onClick} className={cn('flex flex-col items-center justify-center rounded-2xl px-3 py-2 transition', active ? 'bg-neutral-900 text-white' : 'bg-neutral-100 text-neutral-500')}>
