...
사용자가 "MD 업데이트해줘"라고 하면 아래 순서대로 실행:

1. **새 실수/주의사항** — 이번 세션에서 틀렸거나 헤맨 것 → `자주 발생한 실수 & 주의사항` 섹션에 추가
2. **설정 변경** — 환경변수/외부서비스/배포 설정 바뀐 것 → 해당 섹션 업데이트
3. **새 파일** — 이번 세션에서 추가된 파일 → `주요 파일 맵` 에 추가

확인 후 CLAUDE.md 수정 → git commit → push까지 한 번에 처리.
